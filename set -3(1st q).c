...
void subsetSumUtil(int set[], int subset[], int n, int sum, int currentSum, int index, int subsetSize) {
    if (currentSum == sum) {
    ...
    if (index < n && currentSum + set[index] <= sum) {
        ...
        subsetSumUtil(set, subset, n, sum, currentSum + set[index], index + 1, subsetSize + 1);
    }

    // Exclude the current element and try the next one
    if (index < n) {
        subsetSumUtil(set, subset, n, sum, currentSum, index + 1, subsetSize);
    }
}
...
void findSubsetsWithSum(int set[], int n, int sum) {
    ...
    subsetSumUtil(set, subset, n, sum, 0, 0, 0);
}
...
int main() {
    ...
    int set2[] = {6, -4, -1, 5, 2, 8, 1};
    int n2 = sizeof(set2) / sizeof(set2[0]);
    int sum2 = 10;

    printf("\nInput Set(s) = (6, -4, -1, 5, 2, 8, 1) Sum is 10\n");
    printf("Output:\n");
